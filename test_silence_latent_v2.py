...
silence_latent = ace.silence_latent[:, :num_frames, :]
...
print("\nGenerating audio with DiT (using silence latent)...")
print(f"GPU memory before DiT: {torch.cuda.memory_allocated(1e9):.2f} GB")
...
print(f"GPU memory after DiT: {torch.cuda.memory_allocated(1e9):.2f} GB")

# 清理DiT相关内存
del text_hidden, text_mask, lyric_hidden, lyric_mask, refer_audio_hidden, refer_audio_mask
del src_latents, attention_mask, chunk_masks, is_covers, hidden_states_for_gen, result
torch.cuda.empty_cache()
gc.collect()
...
sf.write("test_silence_latent.wav", audio_np, 48000)
print("Saved to test_silence_latent.wav")

# 分析音频
import numpy as np
print(f"\nAudio analysis:")
print(f"  RMS: {(audio_np**2).mean()**0.5:.6f}")
print(f"  Peak: {np.abs(audio_np).max():.4f}")
print(f"  Zero crossings: {np.sum(np.abs(np.diff(np.sign(audio_np))))}")
chunk_size = 48000
...
print(f"  Unique values: {len(np.unique(audio_np))}")
